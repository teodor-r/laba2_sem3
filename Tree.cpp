...
template <class TElement>
Node<TElement>::Node(string& key, TElement* el,list<std::string> not_par,int curr_value, int curr_weight,int curr_volume, Node<TElement> *p)
{
    parent = p;
    element.first = key;
    element.second = el;
...
template<class TElement>
Tree<TElement>::Tree(list<std::string> all_elements,const Dictionary<TElement> &dict)
{
...
{   int counter = 0;
    ...
    {   Item* t = dict.Get(*it);
        ...
        {   counter++;
            ...
            Node<Item> *next_node = new Node<Item>(*it, t, not_parents_next_node, ncv, ncw, ncvol, node);
